...
const LeadFollowUpsTable = () => {
  ...
  const getTrimmedText = (text, isExpanded) => {
    if (isExpanded || text.length <= 30) {
      return text;
    }
          ...
          <tbody className="text-sm">
            {followUps &&
              followUps.map((followUp) => {
                const isExpanded = expandedRows[followUp._id];
                return (
                  <tr key={followUp._id}>
                    ...
                    <td
                      className="cursor-pointer border-b border-[#eee] px-1 py-1.5 dark:border-strokedark"
                      onClick={() => toggleRowExpansion(followUp._id)}
                    >
                      <p className="text-black dark:text-white">
                        {getTrimmedText(followUp.remarks, isExpanded)}
                      </p>
                    </td>
                  </tr>
                );
